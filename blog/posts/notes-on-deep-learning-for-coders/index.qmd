...
**Accuracy** is a metric used to evaluate the performance of a model by measuring the percentage of correct predictions made by the model. It uses the training set’s label (independent variable) as the reference. Note that accuracy is only used in classification models and not in regression models. 
